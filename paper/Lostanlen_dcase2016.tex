...
dimensionless frequency $\xi$ and bandwidth $\xi/Q$.
...
log-frequency index $k_1$ takes integer values in the interval
$\llbracket 0 ; K_1 \llbracket$.
...
= \vert \boldsymbol{y_1}[t, k_1] \vert$, indexed by time $t$ and
log-frequency $k_1$.
...
\end{equation}
where the two-dimensional filter
$\boldsymbol{\Psi_{k_2}}[t,k_1]$ is either
a temporal low-pass filter $\boldsymbol{\phi}[t]$,
a oriented edge detector in the time-frequency plane
$(\boldsymbol{\psi_{\alpha}}[t] \times 
\boldsymbol{\psi_{\beta}}[k_1])$,
or 
$(\boldsymbol{\psi_{\alpha}}[t] \times
\boldsymbol{\phi}[k_1])$.

\begin{equation}
\mathbf{W_2}[t, k_1, k_2] =
\alpha \psi(\alpha t)
\times
\vert \beta \vert \psi(\beta k_1)
\end{equation}
