...
% affiliations
\author{\IEEEauthorblockN{Vincent Lostanlen
...
% A challenge in temporal integration

%Our main contribution is to

% Contribution:
% Online data augmentation to enforce invariance to binaural azimuth
...
$k_2$ encapsulates two values, \ie
...
$4$ octaves, as well as their $5$ "mirror" frequencies,
...
& = &
\left( \boldsymbol{x_1} \right. \! \overset{t,k_1}{\ast} \!
\left. \boldsymbol{\Psi_{k_2}} \right) [t, k_1]
\nonumber \\
& = &
\sum_{\tau, \kappa_1}
\boldsymbol{x_1}[t - \tau, k_1 - \kappa_1]
\boldsymbol{\Psi_{k_2}}[\tau, \kappa_1].
\end{IEEEeqnarray}
...
% Fig. 1, (a), (b), (c), (d).
% Show time-frequency scattering coefficients for various
% values of (\alpha, \beta), including the two edge cases.

% Fig. 2 (a), (b), (c), (d).
% Tme-frequency scattering coefficeints.

% Characterization of stationary processes
% Re-synthesis from a time-invariant representation
...
\boldsymbol{y_{3}^{+}}[t,k_{1},k_{3}]=\left\{ \! \! \! \begin{array}{r}
\nu\,\boldsymbol{y_{3}}[t,k_{1},k_{3}] \;\;\; \mbox{if} \;\; \boldsymbol{y_{3}}[t,k_{1},k_{3}]<0\\
\boldsymbol{y_{3}}[t,k_{1},k_{3}] \;\;\; \mbox{if} \;\; \boldsymbol{y_{3}}[t,k_{1},k_{3}]\geq0
\end{array}\right. \!
\IEEEeqnarraynumspace
\label{eq:relu}
 \end{IEEEeqnarray}
...
Data augmentation is a simple, yet effective, way to enforce invariance in feature learning
is to 
\begin{equation}
\boldsymbol{x}[t] =
